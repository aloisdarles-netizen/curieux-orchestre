/* ---------------------------------------------------------------------------
   ...
   Quatre états, dans l'ordre où une date les traverse :
     ...
     recherche — on demande autour de nous qui serait là. RIEN n'est réservé.
                 C'est une hypothèse de travail, souvent large, dont on tirera
                 peut-être une option. C'est le premier état, et il manquait :
                 on était obligé de poser une « option » qui n'en était pas une.
   ...
   Une date en recherche et une date en option sont toutes deux des HYPOTHÈSES :
   c'est ce que dit `estHypothese`. La différence, c'est qu'une option engage la
   salle et pas une recherche.
--------------------------------------------------------------------------- */

const STATUTS_DATE = [
  // `pill` reprend le vocabulaire de la charte (co-pill). Le gris de
  // « recherche » est délibéré : rien n'est engagé, il n'y a rien à colorer.
  { cle:'recherche', libelle:'Recherche de dispo', court:'Recherche', pill:'',    ordre:0 },
...
// « recherche » : on ne dégrade pas une date que quelqu'un a posée.
...
// Le cycle de la pastille, dans l'ordre du tableau : recherche → option →
// validée → annulée → recherche.
